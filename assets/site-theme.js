(()=>{
  ...
  const loadStyle=(href,attr)=>{
    ...
    link.rel='stylesheet';link.href=href;link.dataset[attr]='true';
    document.head.appendChild(link);
  };
  const loadAsset=(href,attr)=>{
    if(document.querySelector(`script[data-${attr}]`))return;
    const script=document.createElement('script');
    script.src=href;script.defer=true;script.dataset[attr]='true';
  ...
  const init=()=>{
    const vt=isVinTech();
    loadStyle('/assets/theme-runtime-2026.css','theme-runtime');
    loadStyle('/assets/unified-polish-2026.css','unified-polish');
    loadStyle('/assets/ultra-2026.css','ultra-ui');
    ...
    document.querySelectorAll('.mobile-menu').forEach(btn=>{
      const nav=btn.closest('.site-nav')||document.querySelector('.site-nav');
      ...
      btn.onclick=()=>{const open=document.body.classList.toggle('menu-open');btn.setAttribute('aria-expanded',String(open))};
    ...
    document.querySelectorAll('.vt-menu').forEach(btn=>{
      const nav=document.getElementById(btn.getAttribute('aria-controls')||'vt-navigation');
      if(!nav)return;
    ...
    document.querySelectorAll('.nav-links a,.mobile-panel a,.vt-links a').forEach(a=>{
      try{const target=new URL(a.href,location.origin).pathname.toLowerCase().replace(/\/$/,'')||'/';if(target===current)a.setAttribute('aria-current','page')}catch{}
    });
    let ticking=false;
    const progress=()=>{const h=document.documentElement.scrollHeight-innerHeight;root.style.setProperty('--scroll-progress',h>0?String(Math.min(1,Math.max(0,scrollY/h))):'0');ticking=false};
    ...
    progress();
    document.querySelectorAll('.card,.service-card,.project-card,.post-card,.metric,.stat').forEach(card=>card.addEventListener('pointermove',e=>{const r=card.getBoundingClientRect();card.style.setProperty('--card-x',(e.clientX-r.left)+'px');card.style.setProperty('--card-y',(e.clientY-r.top)+'px')},{passive:true}));
  };
  if(media.addEventListener)media.addEventListener('change',()=>{if(!isVinTech()&&!store.get())apply(getTheme())});
  else media.addListener(()=>{if(!isVinTech()&&!store.get())apply(getTheme())});
