# FINAL PASS: deterministic, idempotent normalization for every published HTML page.
...
def english_equivalent(path: Path) -> str | None:
    rel = path.as_posix()
    if rel == 'index.html': return BASE + '/en.html'
    if rel == 'en.html': return BASE + '/'
...
def ensure_head(text: str, path: Path) -> str:
    ...
    if not re.search(r'<meta\s+name=["\']viewport["\']', text, re.I): text = text.replace('<head>', '<head><meta name="viewport" content="width=device-width,initial-scale=1">', 1)
    text = CSS_RE.sub('', text); text = CSS_RE2.sub('', text)
    bundle = '<link rel="stylesheet" href="/assets/site-bundle.css">'
    if bundle not in text: text = text.replace('</head>', bundle + '</head>', 1)
    ...
    def img_repl(m):
        ...
        if 'loading=' not in attrs and 'fetchpriority="high"' not in attrs: attrs += ' loading="lazy"'
        return '<img' + attrs + '>'
    ...
    if path.name != '404.html' and not re.search(r'<link\s+rel=["\']canonical["\']', text, re.I): text = text.replace('</head>', f'<link rel="canonical" href="{canonical}"></head>', 1)
    if path.name == '404.html':
        if not re.search(r'<meta\s+name=["\']robots["\']', text, re.I): text = text.replace('</head>', '<meta name="robots" content="noindex,follow"></head>', 1)
    elif not re.search(r'<meta\s+name=["\']robots["\']', text, re.I): text = text.replace('</head>', '<meta name="robots" content="index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1"></head>', 1)
    alt = english_equivalent(path)
    text = re.sub(r'\s*<link\s+rel=["\']alternate["\']\s+hreflang=["\'][^"\']+["\'][^>]*>', '', text, flags=re.I)
    if path.name != '404.html':
        this = html_url(path)
        links = [f'<link rel="alternate" hreflang="{lang}-IR" href="{this}">'] if lang == 'fa' else [f'<link rel="alternate" hreflang="en" href="{this}">']
        if alt: links.append(f'<link rel="alternate" hreflang="en" href="{alt}">' if lang == 'fa' else f'<link rel="alternate" hreflang="fa-IR" href="{alt}">')
...
changed = []
for p in HTMLS:
    old = p.read_text(encoding='utf-8'); new = ensure_head(old, p)
    if new != old: p.write_text(new, encoding='utf-8'); changed.append(p.as_posix())
