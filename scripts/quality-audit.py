...
class Audit(HTMLParser):
    def __init__(self):
        super().__init__(); self.title=self.desc=self.robots=self.viewport=self.h1=0; self.canonical=[]; self.lang=None; self.dir=None; self.imgs=[]; self.links=[]; self.ids=set(); self.dup=[]; self.scripts=[]; self.styles=[]
...
for p in PAGES:
    ...
    if rel!='404.html' and (len(a.canonical)!=1 or not a.canonical[0].startswith('https://mowhmmdh.github.io/')): ERRORS.append(f'{rel}: canonical invalid')
...
print('PASS: HTML structure, SEO foundations, local links/assets, accessibility basics and secret patterns')
