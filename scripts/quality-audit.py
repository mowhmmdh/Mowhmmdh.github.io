...
class Audit(HTMLParser):
    def __init__(self):
        super().__init__(); self.title=self.desc=self.robots=self.viewport=self.h1=self.author=self.main=0; self.canonical=[]; self.hreflang=[]; self.lang=None; self.dir=None; self.imgs=[]; self.links=[]; self.ids=set(); self.dup=[]; self.scripts=[]; self.styles=[]; self.buttons=[]; self.inputs=[]
...
for p in PAGES:
    ...
    if rel!='404.html' and a.main!=1: ERRORS.append(f'{rel}: main count={a.main}')
    ...
    for b in a.buttons:
        if not b.get('aria-label') and not b.get('title'): WARNINGS.append(f'{rel}: button may lack an accessible name')
