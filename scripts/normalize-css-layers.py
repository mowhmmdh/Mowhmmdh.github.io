...
for path in pages:
    ...
    for legacy in LEGACY:
        if legacy in links:
            errors.append(f'{rel}: legacy stylesheet still linked: {legacy}')
    ...
    modern = [m.group(2) for m in SCRIPT.finditer(text) if 'modern-ui-2026.js' in m.group(2)]
    if modern and not is_vt:
