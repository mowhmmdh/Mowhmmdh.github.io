...
def normalize(path):
    ...
    if not other or not (root/other).exists():
        ...
        return
    en=is_en(path); lang='en' if en else 'fa-IR'; other_lang='fa-IR' if en else 'en'; own=canonical(path); other_url=canonical(other)
    # x-default follows the Persian counterpart for bilingual content, rather than
    # collapsing every section to the homepage.
    x_default=other_url if en else own
